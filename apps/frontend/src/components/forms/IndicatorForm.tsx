...
export const IndicatorForm = ({
  ...
  const isReadOnly = isReviewMode || status === 'Aprobado' || status === 'En revisión';
  ...
  return (
    <form
      ...
      className="w-full max-w-[1250px] mx-auto bg-brand-Blanco rounded-lg shadow-md border border-brand-Gris_bajo/20 p-6 mb-10"
    >
      {/* Cabecera */}
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-3">
            ...
            {status && (
              <span className={`text-[11px] font-bold font-accent px-3 py-1 rounded-full shadow-xs tracking-wide ${
                status === 'Aprobado' ? 'bg-brand-Status_verde text-brand-Gris_oscuro' :
                status === 'En revisión' ? 'bg-brand-Status_azul text-brand-Blanco' :
                status === 'Corregir' ? 'bg-brand-Status_rojo text-brand-Blanco' :
                'bg-brand-Status_amarillo text-brand-Gris_oscuro'
              }`}>
                {status}
              </span>
            )}
      ...
      <div className="mt-8 bg-brand-Gris_bajo/5 p-6 rounded-lg border border-brand-Gris_bajo/20">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className={`flex items-center w-full h-[46px] border border-brand-Gris_bajo/40 rounded-md bg-brand-Blanco overflow-hidden focus-within:border-brand-Verde_principal focus-within:ring-1 focus-within:ring-brand-Verde_principal ${isReadOnly ? 'opacity-70 bg-brand-Gris_bajo/10 cursor-not-allowed' : ''}`}>
            <label className={`flex items-center justify-center h-full ${isReadOnly ? 'bg-brand-Gris_oscuro/40 cursor-not-allowed' : 'cursor-pointer bg-brand-Verde_principal hover:bg-brand-Verde_oscuro'} text-brand-Blanco px-4 text-sm font-bold transition-colors whitespace-nowrap`}>
              {isReadOnly ? 'Archivo Adjunto' : 'Buscar Archivo'}
                <input
                  type="file"
                  accept=".pdf"
                  className="hidden"
            ...
            <span className="text-sm font-body text-brand-Gris_oscuro truncate px-4" title={watchedEvidencia && watchedEvidencia.length > 0 ? watchedEvidencia[0].name : (isReadOnly ? 'Archivo_Evidencia.pdf' : 'Ningún archivo seleccionado')}>
              {watchedEvidencia && watchedEvidencia.length > 0 ? watchedEvidencia[0].name : (isReadOnly ? 'Archivo_Evidencia.pdf' : 'Ningún archivo seleccionado')}
              </span>
            </div>
      ...
      {/* Ocultar completamente los controles inferiores si el usuario no puede interactuar */}
      {(!isReadOnly || (isReviewMode && status !== 'Aprobado')) && (
