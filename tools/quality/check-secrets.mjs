...
for (const relativePath of trackedFiles) {
  ...
  for (const match of content.matchAll(connectionPattern)) {
    const credential = `${match[1]}:${match[2]}`.toLowerCase();
    const explicitPlaceholder = /\$\{|change|example|sample|dummy|synthetic|test|demo|qa[_-]|password|local/.test(credential);
